...
public class CustomTextField {
    ...
    public void render(int n, int n2, float f, float f2) {
        ...
        Rynox.Companion.getFontManager().getFontRenderer().drawString((value.isEmpty() ? text : value) + (active ? typeIndicator() : ""), (x + 2.5f - Math.max(Rynox.Companion.getFontManager().getFontRenderer().getStringWidth(value) * f2 + 5 - (width - 2.5f), 0)) / f2, (y + height / 2 - Rynox.Companion.getFontManager().getFontRenderer().getStringHeight() * f2 / 1.5f) / f2, value.isEmpty() ? new Color(0.5f, 0.5f, 0.5f, f) : new Color(1, 1, 1, f));
        matrixStack.pop();
        GL11.glDisable(3089);
    }
