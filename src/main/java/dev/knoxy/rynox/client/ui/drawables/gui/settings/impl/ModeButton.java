package dev.knoxy.rynox.client.ui.drawables.gui.settings.impl;
    ...
    public void renderButton(int n, int n2, float f, float f2, float f3, float f4, float f5, float f6) {
        ...
        if (anims.getAnimationFactor() > 0.001f) {
            ...
            for (int i = 0; i < stringArray.length; ++i) {
                ...
                Rynox.Companion.getFontManager().getFontRenderer().drawString(string, (getX() + 15) / f25, (f26 + 7.5f - Rynox.Companion.getFontManager().getFontRenderer().getStringHeight() / 1.5f) / f25, RenderUtil.getColor(n > getX() && n < getX() + getWidth() && n2 > f26 && n2 < f26 + 15 ? 1 : 0.5f, f2));
                matrixStack.pop();
                ...
                if (setting.getObject().equals(string)) {
                    RenderUtil.renderFilledCircle(getX() + 5, f26 + 7.5f, 2.5f, RenderUtil.getColor(Color.WHITE, f2));
                }
                f26 += 15;
            }
            GL11.glDisable(3089);
